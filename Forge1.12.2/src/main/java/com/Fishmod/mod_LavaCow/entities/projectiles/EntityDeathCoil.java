        ...
        if (!this.world.isRemote && result != null && result.typeOfHit == RayTraceResult.Type.ENTITY) {
            if (target != null && shooter != null && target instanceof EntityLivingBase && target != shooter && !target.isOnSameTeam(shooter)) {
                ...
                if (target.attackEntityFrom(DamageSource.causeIndirectMagicDamage(this, (EntityLivingBase) shooter).setProjectile(), this.getDamage())) {
                    ...
                    ((EntityLivingBase) target).addPotionEffect(new PotionEffect(MobEffects.WITHER, 10 * 20 + difficulty_calculation, 2));

                    if (shooter instanceof EntityForsaken && ((EntityForsaken) shooter).getOwner() instanceof EntitySkeletonKing) {
                        ((EntityLivingBase) target).addPotionEffect(new PotionEffect(ModMobEffects.FRAGILE_KING, 10 * 20 + difficulty_calculation, 2));
                    } else {
                        ((EntityLivingBase) target).addPotionEffect(new PotionEffect(ModMobEffects.FRAGILE, 10 * 20 + difficulty_calculation, 2));
                    }
