...
public class EntityAIPickupMeat<T extends EntityItem> extends EntityAITarget {
	...
	public void updateTask() {
		...
		if (this.targetEntity != null && !this.targetEntity.isDead && this.taskOwner.getDistanceSq(this.targetEntity) < 1.5D) {
			this.targetEntity.getItem().shrink(1);
			this.taskOwner.playSound(SoundEvents.ENTITY_GENERIC_EAT, 1, 1);
			this.taskOwner.heal(this.taskOwner.getMaxHealth() * 0.1F);
