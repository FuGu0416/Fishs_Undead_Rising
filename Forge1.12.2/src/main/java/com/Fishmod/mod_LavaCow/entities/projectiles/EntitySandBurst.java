            ...
            if (this.clientSideAttackStarted)
            {
                ...
                if (this.lifeTicks == 14)
                {
                    for (int i = 0; i < 24; ++i)
                    {
                        ...
                        double d3 = (this.rand.nextDouble() * 4.0D - 1.0D) * 0.5D;
                        double d4 = 0.5D + this.rand.nextDouble() * 0.5D;
                        double d5 = (this.rand.nextDouble() * 4.0D - 1.0D) * 0.5D;
                        this.world.spawnParticle(EnumParticleTypes.CRIT, d0, d1 + 1.0D, d2, d3, d4, d5);
                        
            	        if (state.isOpaqueCube()) {
    	            		this.world.spawnParticle(EnumParticleTypes.BLOCK_CRACK, d0, d1 + 1.0D, d2, d3, d4, d5, blockId);
            	        }
                    }
                }
            }
