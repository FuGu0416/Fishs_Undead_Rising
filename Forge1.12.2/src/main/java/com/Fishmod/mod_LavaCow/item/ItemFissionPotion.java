...
public class ItemFissionPotion extends ItemFishCustom {
    ...
    public boolean itemInteractionForEntity(ItemStack stack, EntityPlayer playerIn, EntityLivingBase target, EnumHand hand) {	
    	if(((!Modconfig.Fission_ModEntity && LootTableHandler.FISSION_WHITELIST.contains(EntityList.getKey(target))) || Modconfig.Fission_ModEntity && target instanceof EntityLiving) && !target.isChild()) {
    		...
    		if(!playerIn.world.isRemote) {
	    		...
	    		else if(stack.getItem().equals(FishItems.POTION_OF_MOOTEN_LAVA) && !(target instanceof EntityLavaCow) && target instanceof EntityCow) {
	    			
	    			if(target.getActivePotionEffect(MobEffects.FIRE_RESISTANCE) == null) {
	    				target.setFire(12);
	    			}
	    			else {	    				
	    				for(int i = 0 ; i < 2 ; i++) {
    ...
    public ItemStack onItemUseFinish(ItemStack stack, World world, EntityLivingBase entityLiving)
    {
        if (!world.isRemote) {
        	...
        	} else if (stack.getItem().equals(FishItems.POTION_OF_MOOTEN_LAVA) && entityLiving.getActivePotionEffect(MobEffects.FIRE_RESISTANCE) == null) {
