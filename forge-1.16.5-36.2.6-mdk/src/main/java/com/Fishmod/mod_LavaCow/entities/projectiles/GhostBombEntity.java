...
public class GhostBombEntity extends ProjectileItemEntity {
	...
	protected void onHit(RayTraceResult result) {
        ...
        if (!this.level.isClientSide()) {
        	...
        	Dummy.remove();
        	this.level.playSound(null, this.blockPosition(), FURSoundRegistry.BANSHEE_HURT, SoundCategory.BLOCKS, 1.0F, (this.random.nextFloat() - this.random.nextFloat()) * 0.2F + 1.0F);
        	this.remove();
        }
	}
