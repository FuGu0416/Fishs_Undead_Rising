...
import com.Fishmod.mod_LavaCow.entities.flying.RidableFlyingMobEntity;
...
public class MessageMountSpecial {
	...
	public static class Handler implements BiConsumer<MessageMountSpecial, Supplier<NetworkEvent.Context>> {
		...
		public void accept(MessageMountSpecial message, Supplier<Context> context) {
			...
			// Only allow the special attack while the sender is actually riding the target entity, and
			// always spawn effects from the entity's own server-side position/rotation - never trust the
			// client-supplied coordinates in the packet.
			...
			if (entity instanceof SalamanderEntity) {
				...
				if (salamander.isBarrageOnCooldown()) {
					return;
				}
				salamander.setBarrageCooldown(80);
		   	 	...
		   	 	for (int i = 0 ; i < 8 ; i++) {
		   	 		...
		   	 		entityammo.setPos(posX + lookVec.x * 2.0D, posY + (double)(entity.getBbHeight() / 2.0F) + 1.5D, posZ + lookVec.z * 2.0D);
					entity.level.addFreshEntity(entityammo);
		   	 	}
		   	 	entity.level.playSound(null, posX, posY, posZ, SoundEvents.BLAZE_SHOOT, SoundCategory.PLAYERS, 1.0F, 1.0F / (new Random().nextFloat() * 0.4F + 1.2F));
			} else if (entity instanceof RidableFlyingMobEntity) {
				RidableFlyingMobEntity mount = (RidableFlyingMobEntity) entity;
				if (mount.abilityCooldown > 0) {
					return;
				}
				mount.abilityCooldown = mount.abilityCooldown();
				...
				if (entity instanceof VespaEntity) {
					...
					entity.level.broadcastEntityEvent(entity, (byte)10);
			   	 	...
			   	 	for (int i = 0 ; i < 5 ; i++) {
			   	 		...
			   	 		entityammo.setPos(posX - entity.getBbWidth() + (entity.getBbWidth() * player.getRandom().nextDouble()), posY - (double)(entity.getBbHeight() / 2.0F), posZ - entity.getBbWidth() + (entity.getBbWidth() * player.getRandom().nextDouble()));
			   	 		entity.level.addFreshEntity(entityammo);
			   	 		entityammo.setScaleType(((EnigmothEntity) entity).getSkin());
			   	 	}
			   	 	entity.level.playSound(null, posX, posY, posZ, SoundEvents.EVOKER_CAST_SPELL, SoundCategory.PLAYERS, 1.0F, 1.0F / (new Random().nextFloat() * 0.4F + 1.2F));
			   	 	entity.level.broadcastEntityEvent(entity, (byte)10);
