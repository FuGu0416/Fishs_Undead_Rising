...
public class EntityAITargetItem<T extends ItemEntity> extends TargetGoal {
    ...
    public EntityAITargetItem(Mob creature, Class<T> classTarget, boolean checkSight) {
        this(creature, classTarget, checkSight, false);
    }

    public EntityAITargetItem(Mob creature, Class<T> classTarget, boolean checkSight, boolean onlyNearby) {
        this(creature, classTarget, 10, checkSight, onlyNearby, (Predicate<? super ItemEntity>)null);
    }

    public EntityAITargetItem(Mob creature, Class<T> classTarget, int chance, boolean checkSight, boolean onlyNearby, @Nullable final Predicate <? super T > targetSelector) {
    ...
    public boolean canUse() {
        if ((this.mob.isPassenger() || (mob.isVehicle() && mob.getControllingPassenger() != null)) && (this.mob instanceof TamableAnimal && ((TamableAnimal) this.mob).isInSittingPose()) || (!this.mob.getMainHandItem().isEmpty() && this.targetChance > 0 && this.mob.getRandom().nextInt(this.targetChance) != 0)) {
            return false;
        } else {
	...
	public void tick() {
        ...
        if (targetEntity != null && this.mob.hasLineOfSight(targetEntity) && this.mob.onGround()) {
